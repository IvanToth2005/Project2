...
class DatabaseManager {
    ...
    public function getAllData() {
        ...
        products.name AS product_name, products.id AS id
        FROM stores
    ...
    public function getLowStockProducts() {
        $sql = "SELECT products.name AS product_name, products.quantity
                FROM products
                WHERE products.quantity < products.min_qty";
    ...
    public function modifyDataByProductId($productId, $modifiedData) {
        $stmt = $this->conn->prepare("UPDATE products 
                                       INNER JOIN shelves ON products.id_shelf = shelves.id
                                       INNER JOIN columns ON shelves.id_column = columns.id
                                       INNER JOIN `rows` ON columns.id_row = `rows`.id
                                       INNER JOIN stores ON `rows`.id_store = stores.id
                                       SET stores.name = ?, 
                                           stores.address = ?, 
                                           shelves.name = ?, 
                                           `rows`.name = ?, 
        ...
        $stmt->bind_param("ssssssiiii", 
                          $modifiedData['store_name'], 
                          $modifiedData['store_address'], 
                          $modifiedData['shelf_name'], 
                          $modifiedData['row_name'], 
                          $modifiedData['column_name'], 
                          $modifiedData['product_name'], 
                          $modifiedData['min_qty'], 
                          $modifiedData['quantity'], 
                          $modifiedData['price'], 
                          $productId);
        $stmt->execute();
    }
